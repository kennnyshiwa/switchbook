...
async function main() {
  ...
  await linkSourceReviewGroup({reviewIds:exact80Reviews.map(r=>r.id),masterSwitchId:exact80Master.id,catalogEntryId:exact80High.id,actorId:user.id},prisma).then(()=>assert.fail('manual decision was silently superseded'),error=>assert.equal((error as Error).message,'CONFLICTING_APPROVED_MAPPING'))
  assert.equal(JSON.stringify(await prisma.forceCurveReviewCase.findMany({where:{id:{in:exact80Reviews.map(r=>r.id)}},orderBy:{id:'asc'}})),exact80Before)
  await prisma.forceCurveMapping.delete({where:{masterSwitchId_catalogEntryId:{masterSwitchId:exact80Master.id,catalogEntryId:exact80Raw.id}}})
  ...
  assert.equal((await getApprovedCurves(exact80Master.id)).length,1)
