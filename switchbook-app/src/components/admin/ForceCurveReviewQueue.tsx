...
import { forceCurveAttachErrorMessage } from '@/lib/admin-force-curve-attach-feedback'
...
type AttachFeedback = { status: 'loading' | 'error' | 'success'; message: string; refreshable?: boolean }
...
export default function ForceCurveReviewQueue({ initialQueue, rankAssistEnabled = false }: { initialQueue: Queue; rankAssistEnabled?: boolean }) {
  ...
  const [attachFeedback, setAttachFeedback] = useState<Record<string, AttachFeedback>>({})
  const [openSwitchesDB, setOpenSwitchesDB] = useState<string | null>(null)
  ...
  async function chooseMaster(item: Item) {
    ...
    if (!catalog || !masterSwitchId) {
      setAttachFeedback(value => ({ ...value, [item.sourceKey]: { status: 'error', message: 'Select a catalog candidate and MasterSwitch.' } }))
      return
    }
    setBusy(item.sourceKey)
    setError('')
    setAttachFeedback(value => ({ ...value, [item.sourceKey]: { status: 'loading', message: 'Attaching selected MasterSwitch…' } }))
    try {
      ...
      const data = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(forceCurveAttachErrorMessage(data.error))
      setAttachFeedback(value => ({ ...value, [item.sourceKey]: { status: 'success', message: 'MasterSwitch attached successfully.' } }))
      if (selectedMaster) {
      ...
      try {
        ...
        setAttachFeedback(value => ({ ...value, [item.sourceKey]: { status: 'success', message: 'MasterSwitch attached successfully, but the queue refresh timed out. Reload the page to fetch the latest queue state.' } }))
      }
    } catch (e) {
      const message = e instanceof Error ? e.message : 'Link failed'
      setAttachFeedback(value => ({ ...value, [item.sourceKey]: { status: 'error', message, refreshable: message === forceCurveAttachErrorMessage('INCOMPLETE_SOURCE_GROUP') } }))
    } finally {
      setBusy('')
    }
  }
    ...
    <section className="space-y-5" aria-busy={Boolean(busy)}>
      ...
      <div className="space-y-4">
        {queue.items.map(item => {
          ...
          const switchesDB = resolveSwitchesDBMeasurement(first.candidates, candidate?.id)
          const cardAttachFeedback = attachFeedback[item.sourceKey]
          return (
              ...
              <div className="grid md:grid-cols-2">
                <div className="border-b border-gray-200 p-4 sm:p-6 md:border-b-0 md:border-r dark:border-gray-700">
                  ...
                  <div className="mt-3 flex flex-col items-start gap-2">
                    {switchesDB.available
                      ? <ForceCurveLookupButton url={switchesDB.url} label={switchesDB.label} open={openSwitchesDB === item.sourceKey} onOpenChange={open => setOpenSwitchesDB(open ? item.sourceKey : null)} />
                      : <p data-testid={`switchesdb-unavailable-${item.sourceKey}`} role="status" className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-100">{switchesDB.reason}</p>}
                    <a className="inline-flex min-h-11 items-center rounded-md text-sm font-medium text-blue-700 underline decoration-blue-300 underline-offset-4 hover:text-blue-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 dark:text-blue-300 dark:hover:text-blue-100 dark:focus-visible:ring-offset-gray-800" href={sourceLink.href} target="_blank" rel="noopener noreferrer" aria-label={`Open ${sourceLink.publisher} ${sourceLink.exactFile ? 'force curve source file' : 'force curve repository'} for ${candidate?.displayName || item.sourceKey} in a new tab`}>View source on GitHub · {sourceLink.publisher}</a>
                  </div>
