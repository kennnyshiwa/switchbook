...
type AttachFeedback = { status: 'loading' | 'error' | 'success'; message: string }
...
export default function ForceCurveReviewQueue({ initialQueue, rankAssistEnabled = false }: { initialQueue: Queue; rankAssistEnabled?: boolean }) {
  ...
  async function chooseMaster(item: Item) {
    ...
    try {
      ...
      setAttachFeedback(value => ({ ...value, [item.sourceKey]: { status: 'error', message: e instanceof Error ? e.message : 'Link failed' } }))
    } finally {
      setBusy('')
    }
  }
