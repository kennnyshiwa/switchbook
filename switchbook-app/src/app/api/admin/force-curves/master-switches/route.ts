...
import { resolveUniqueCatalogMaster, resolvedCatalogMasterCompatibility } from '@/lib/admin-force-curves'
...
export async function GET(request: NextRequest) {
  ...
  const [matches, resolution] = await Promise.all([
    prisma.masterSwitch.findMany({ where: { status: 'APPROVED', OR: [{ id: query }, { name: { contains: query, mode: 'insensitive' } }, { manufacturer: { contains: query, mode: 'insensitive' } }] }, select: { id: true, name: true, manufacturer: true, technology: true, type: true }, orderBy: [{ manufacturer: 'asc' }, { name: 'asc' }, { id: 'asc' }], take: 50 }),
