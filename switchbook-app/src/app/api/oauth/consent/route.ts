...
import { PARTNER_SCOPES } from '@/lib/partner-api/config'

type ConsentRequest = { requested_scope: string[]; client: { client_name?: string; client_id: string }; subject: string }
...
export async function GET(request: NextRequest) {
  ...
  const consent = await hydraAdmin<ConsentRequest>(`/admin/oauth2/auth/requests/consent?consent_challenge=${encodeURIComponent(challenge)}`)
  if (consent.subject !== session.user.id) return NextResponse.json({ error: 'OAuth subject mismatch' }, { status: 403 })
  const allowed = consent.requested_scope.filter(scope => (PARTNER_SCOPES as readonly string[]).includes(scope) || ['openid','offline_access'].includes(scope))
...
export async function POST(request: NextRequest) {
  ...
  const allowed = requested.filter(scope => consent.requested_scope.includes(scope) && ((PARTNER_SCOPES as readonly string[]).includes(scope) || ['openid','offline_access'].includes(scope)))
  const decision = form.get('decision')
  const endpoint = decision === 'allow' ? 'accept' : 'reject'
  const body = decision === 'allow' ? {
    grant_scope: allowed, remember: false,
    session: { id_token: { username: session.user.name, email: session.user.email }, access_token: { client_id: consent.client.client_id } },
