...
export default function ShareUserSwitchPageClient() {
  ...
  useEffect(() => { fetch(`/api/share/user-switch/${shareableId}`).then(async response => { if (!response.ok) throw new Error(response.status === 404 ? 'Switch not found' : 'Failed to load switch'); setShare(serializeCanonicalSwitchShare('user', await response.json())) }).catch(reason => setError(reason.message)) }, [shareableId])
