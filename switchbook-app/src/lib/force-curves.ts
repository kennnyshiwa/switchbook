...
export async function syncForceCurveCatalog(revision: string, entries: CatalogInput[], options: { chunkSize?: number; failAfterChunks?: number; failAfterReconcileChunks?: number; catalogRevision?: string } = {}) {
  ...
  try {
    ...
    run = await prisma.$transaction(async tx => {
      ...
      await tx.$executeRaw`UPDATE "ForceCurveMapping" m SET "state" = 'AUTO_APPROVED'::"ForceCurveMappingState", "updatedAt" = NOW() WHERE m."state" = 'REVIEW_REQUIRED'::"ForceCurveMappingState" AND EXISTS (SELECT 1 FROM "ForceCurveSyncStage" s WHERE s."runId" = ${run.id} AND s."outputType" = 'MAPPING' AND s."masterSwitchId" = m."masterSwitchId" AND s."catalogEntryId" = m."catalogEntryId")`
